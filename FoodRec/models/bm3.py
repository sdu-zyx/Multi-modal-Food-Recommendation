...
from FoodRec.common.loss import EmbLoss
...
class BM3(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.train_interactions = self.dataset.train_coo_matrix

        self.v_feat = torch.tensor(self.dataset.embImage.astype(np.float32)).to(self.device)
        self.t_feat = torch.tensor(self.dataset.embText.astype(np.float32)).to(self.device)

        self.n_nodes = self.n_users + self.n_items

        # load dataset info
        self.norm_adj = self.get_norm_adj_mat(self.train_interactions).to(self.device)
        ...
        if self.v_feat is not None:
            ...
            self.image_trs = nn.Linear(self.v_feat.shape[1], self.emb_size)
            nn.init.xavier_normal_(self.image_trs.weight)
        if self.t_feat is not None:
            self.text_embedding = nn.Embedding.from_pretrained(self.t_feat, freeze=False)
            self.text_trs = nn.Linear(self.t_feat.shape[1], self.emb_size)
    ...
    def calculate_loss(self, batch_data):
        ...
        u_online_ori, i_online_ori = self.gnn_encode()
        ...
        return (loss_ui + loss_iu).mean(), self.reg_weight * self.reg_loss(u_online_ori, i_online_ori), self.cl_weight * (loss_t + loss_v + loss_tv + loss_vt).mean()

    def inference_by_user(self, user_batch):
        user = user_batch['user_input']
        item = user_batch['item_input']
        u_online, i_online = self.gnn_encode()
        u_online, i_online = self.predictor(u_online), self.predictor(i_online)
        score_mat_ui = torch.mul(u_online[user], i_online[item]).sum(dim=1)
